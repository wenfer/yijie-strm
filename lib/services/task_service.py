...
class StrmTask:
    ...
    overwrite_strm: bool = False  # 是否覆盖已存在的 STRM 文件
    ...
    def to_dict(self) -> Dict:
        ...
        data['overwrite_strm'] = 1 if self.overwrite_strm else 0
    ...
    def from_dict(cls, data: Dict) -> 'StrmTask':
        ...
        data['overwrite_strm'] = bool(data.get('overwrite_strm', 0))
...
class TaskService:
    ...
    def create_task(self, task_data: Dict) -> StrmTask:
        ...
        task = StrmTask(
            ...
            overwrite_strm=task_data.get('overwrite_strm', False),
            created_at=now,
            updated_at=now
        )
        ...
        logger.info(f"task_service.py:156 - Created task: {task_id}")
    ...
    def update_task(self, task_id: str, updates: Dict) -> bool:
        ...
        if not task:
            logger.error(f"task_service.py:191 - Task not found: {task_id}")
            return False
        ...
        for field in ['include_video', 'include_audio', 'schedule_enabled',
                      'watch_enabled', 'delete_orphans', 'preserve_structure', 'overwrite_strm']:
            if field in updates:
        ...
        logger.info(f"task_service.py:222 - Updated task: {task_id}")
        return True
    ...
    def delete_task(self, task_id: str) -> bool:
        ...
        logger.info(f"task_service.py:232 - Deleted task: {task_id}")
        return True
    ...
    def add_strm_record(self, task_id: str, file_id: str, file_name: str,
                        file_path: str, strm_path: str, strm_url: str,
                        pick_code: str = None, file_size: int = None) -> bool:
        """添加 STRM 记录"""
        now = time.time()
        record_id = f"{task_id}_{file_id}"

        data = {
            'record_id': record_id,
            'task_id': task_id,
            'file_id': file_id,
            'pick_code': pick_code or file_id,
            'file_name': file_name,
            'file_size': file_size,
            'file_path': file_path,
            'strm_path': strm_path,
        ...
        try:
            ...
            logger.error(f"task_service.py:335 - Failed to add STRM record: {e}")
            return False

    def update_strm_record(self, record_id: str, file_name: str = None,
                           strm_path: str = None, strm_url: str = None) -> bool:
        """更新 STRM 记录"""
        updates = {'updated_at': time.time()}
        ...
        if strm_path:
            updates['strm_path'] = strm_path
        if strm_url:
            updates['strm_content'] = strm_url
        ...
        try:
            ...
            logger.error(f"task_service.py:364 - Failed to update STRM record: {e}")
            return False
    ...
    def delete_strm_record(self, record_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"task_service.py:377 - Failed to delete STRM record: {e}")
            return False
            ...
            if record['file_id'] not in current_file_ids:
                ...
                if os.path.exists(record['strm_path']):
                    try:
                        ...
                        logger.info(f"task_service.py:408 - Deleted orphan STRM file: {record['strm_path']}")
                    except Exception as e:
                        logger.error(f"task_service.py:410 - Failed to delete STRM file: {e}")
    ...
    def create_snapshot(self, task_id: str, files: List[Dict]) -> bool:
        ...
        for file_info in files:
            ...
            try:
                ...
                logger.error(f"task_service.py:456 - Failed to create snapshot for file {file_info['file_id']}: {e}")
    ...
    def add_task_log(self, task_id: str, level: str, message: str) -> bool:
        """添加任务日志（简化版本）"""
        now = time.time()
        log_id = f"{task_id}_{int(now * 1000)}"
        ...
        try:
            ...
            logger.error(f"task_service.py:535 - Failed to add task log: {e}")
            return False
